...
execute store result score @s VETmp.PlayerAttackEvent.sharpnessLevel run data get entity @s SelectedItem.tag.Enchantments[{id:"minecraft:sharpness"}].lvl 1
...
execute at @s if entity @e[type=#ve:arthropods,distance=..9] store result score @s VETmp.PlayerAttackEvent.baneLevel run data get entity @s SelectedItem.tag.Enchantments[{id:"minecraft:bane_of_arthropods"}].lvl 1
#     -> Did the arthropod possibly die from the attack, preventing us from even detecting its presence? Check.
execute unless score @s VE.PlayerAttackEvent.opponentSurvived matches 1 store result score @s VETmp.PlayerAttackEvent.baneLevel run data get entity @s SelectedItem.tag.Enchantments[{id:"minecraft:bane_of_arthropods"}].lvl 1
...
execute at @s if entity @e[type=#ve:undeads,distance=..9] store result score @s VETmp.PlayerAttackEvent.smiteLevel run data get entity @s SelectedItem.tag.Enchantments[{id:"minecraft:smite"}].lvl 1
 #    -> Did the undead possibly die from the attack, preventing us from even detecting its presence? Check.
execute unless score @s VE.PlayerAttackEvent.opponentSurvived matches 1 store result score @s VETmp.PlayerAttackEvent.smiteLevel run data get entity @s SelectedItem.tag.Enchantments[{id:"minecraft:smite"}].lvl 1
